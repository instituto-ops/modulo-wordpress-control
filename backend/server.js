...
const MEMORY_FILE_PATH = path.join(__dirname, 'data/estilo_victor.json');
...
app.post('/api/dna/auto-refine', async (req, res) => {
    try {
        ...
        if (Array.isArray(newRules) && newRules.length > 0) {
            ...
            fs.writeFileSync(path.join(__dirname, 'data/estilo_victor.json'), JSON.stringify(memory, null, 2));
            return res.json({ success: true, newRules });
        }
